...
// define aliases
use db\discipline;
use db\adapterDiscipline;

// add required database tools
require_once(dirname(__FILE__) . "/../adapter/db_adapter_generic.php");
require_once(dirname(__FILE__) . "/../adapter/db_adapter_discipline.php");
...
class managerDiscipline
{
    ...
    public function getDisciplineById(int $id): ?discipline
    {
        ...
        return adapterDiscipline::search($this->db, $id)[0];
    }
    ...
    public function add(discipline $discipline): int|discipline
    {
        ...
        adapterDiscipline::makeRepresentativeDbReady($this->db, $discipline);

        // search for similar disciplines in database
        if (adapterDiscipline::search(
            $this->db,
            null,
    ...
    public function edit(discipline $discipline, array $fields): int
    {
        ...
        // Note: we don't need to check fields since adapterDiscipline::edit() is robust against invalid fields
        ...
        $result = adapterDiscipline::edit($this->db, $discipline, $fields)[0];
    ...
    public function remove(discipline $discipline): int
    {
        ...
        adapterDiscipline::remove($this->db, [$discipline]);
    ...
    private function validate(mysqli $db, discipline $disciplineToValidate): int
    {
        ...
        $found_disciplines = adapterDiscipline::search($db, $disciplineToValidate->{discipline::KEY_ID});
