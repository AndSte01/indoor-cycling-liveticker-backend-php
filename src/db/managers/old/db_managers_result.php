...
class managerResultsOld implements managerResultInterface
{
    ...
    public function getResult(DateTime $modified_since = null): array
    {
        ...
        return adapterResult::search($this->db, null, [$this->currentDisciplineID], $modified_since);
    }
    ...
    {
        // search for result with corresponding parameters
        return adapterResult::search($this->db, $id)[0];
    }
    ...
    {
        // search for result with corresponding parameters
        return adapterResult::searchByCompetition($this->db, $competition_id, $modified_since);
    }
    ...
    public function add(result $result): int|result
    {
        ...
        $result = adapterResult::add($this->db, [$result])[0];
    ...
    public function edit(result $result): int
    {
        ...
        $result = adapterResult::edit($this->db, [$result])[0];
    ...
    public function remove(result $result): int
    {
        ...
        adapterResult::remove($this->db, [$result]);
    ...
    private function validate(mysqli $db, result $resultToValidate): int
    {
        ...
        $found_results = adapterResult::search($db, $resultToValidate->{result::KEY_ID});
