...
require_once(dirname(__FILE__) . "/../adapter/db_adapter_user.php");
...
class managerUser
{
    /** @var int Error at adapter level */
    public const ERROR_ADAPTER = 1;
    ...
    private function validateBinaryToken(user $user, string $token): bool
    {
        ...
        $delta_t = adapterGeneric::getCurrentTime($this->db)->getTimestamp() - $user->{user::KEY_BINARY_TIMESTAMP}->getTimestamp();
    ...
    private function getBinaryToken(user $user): string
    {
        ...
        $user_for_db = new user(
            ...
            null,
            null,
            adapterGeneric::getCurrentTime($this->db), // set time for token generation to current database time
            $new_binary_token
        );

        // edit user in the database
        adapterUser::edit($this->db, $user_for_db, [user::KEY_BINARY_TIMESTAMP, user::KEY_BINARY_TOKEN]);
    ...
    public function getUserByName(string $name): ?user
    {
        ...
        $result = adapterUser::search($this->db, null, $name);
    ...
    public function add(string $username, string $password, int $role): int
    {
        ...
        adapterUser::makeRepresentativeDbReady($this->db, $user);

        // add user to the database, and check for success
        if (adapterUser::add($this->db, [$user]) == null)
    ...
    public function editPassword(string $username, string $password): int
    {
        ...
        // edit user in the database
        adapterUser::edit($this->db, $new_user, [
    ...
    {
        adapterUser::remove($this->db, [new user($id)]);
    }
