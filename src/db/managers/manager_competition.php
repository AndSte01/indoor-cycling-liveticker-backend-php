...
require_once(dirname(__FILE__) . "/../adapter/db_adapter_competition.php");
...
class managerCompetition
{
    ...
    /** @var int Error happened at adapter level */
    const ERROR_ADAPTER = 32;
    ...
    public function getCompetitionById(int $id): ?competition
    {
        ...
        $competitions = adapterCompetition::search($this->db, true, $id);
    ...
    public function getCompetitionsGeneric(?int $daysSinceToday, ?int $limit): array
    {
        ...
        switch (true) {
            case ($daysSinceToday === null):
                $competitions = adapterCompetition::search($this->db, true, null, null, null, null, $limit, null);
                break;

            case ($daysSinceToday == 0):
                $competitions = adapterCompetition::search($this->db, true, null, new DateTime(), null, null, $limit, null);
                break;

            default:
                $date_end = (new DateTime())->modify("-$daysSinceToday day");
                $competitions = adapterCompetition::search($this->db, true, null, new DateTime(), null, null, $limit, $date_end);
                break;
        }
    ...
    public function add(competition $competition): int|competition
    {
        ...
        adapterCompetition::makeRepresentativeDbReady($this->db, $competition);

        // search for similar competition in database to prevent duplicates
        $found_competitions = adapterCompetition::search(
            $this->db,
            true,
            null,
        ...
        $result = (adapterCompetition::add($this->db, [$competition]))[0];
    ...
    public function edit(competition $competition, array $fields): int
    {
        ...
        // Note: we don't need to check fields since adapterCompetition::edit() is robust against invalid fields

        // everything seems fine write the competition to the database
        adapterCompetition::edit($this->db, $competition, $fields);
    ...
    public function remove(competition $competition): int
    {
        ...
        // everything seems fine write the competition to the database
        adapterCompetition::remove($this->db, [$competition]);
    ...
    protected function validate(mysqli $db, competition $competitionToValidate): int
    {
        ...
        $found_competitions = adapterCompetition::search($db, true, $competitionToValidate->{competition::KEY_ID});
