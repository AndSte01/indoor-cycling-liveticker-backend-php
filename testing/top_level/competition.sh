...
URL="http://localhost/liveticker/competition.php"
URL_USER="http://localhost/liveticker/user.php"


# 1 Testing for invalid get parameters
printf "\033[1mTesting for invalid GET parameters\033[0m"

printf "\n\n1.1 expecting no errors (competition retruned)\n"
curl -X GET ${URL}

printf "\n\n1.1 expecting PARAM_OUT_OF_RANGE (caused by method)\n"
curl -X GET ${URL}?method=hello

printf "\n\n1.2 expecting PARAM_OUT_OF_RANGE (caused by id < 1)\n"
curl -X GET ${URL}?competition=0

printf "\n\n1.3 expecting NaN (caused by day)\n"
curl -X GET ${URL}?days=hello

printf "\n\n1.4 expecting NaN (caused by limit)\n"
curl -X GET ${URL}?limit=lovely

printf "\n\n1.5 expecting NaN (caused by id)\n"
curl -X GET ${URL}?id=world
...
printf "\n\n3.1.1: expecting no error (competition as json in return)\n"
